...
void MainWindow::setName()
{
    ...
    QModelIndex idxName = proxyModel->index(idx4.row(), 2);
...
void MainWindow::changeActiveRow(const QModelIndex &index)
{
    ...
    QModelIndex nameIndex = _exampleModel->index(idx4.row(), 2);
    QModelIndex networthIndex = _exampleModel->index(idx4.row(), 3);
    QModelIndex ageIndex = _exampleModel->index(idx4.row(), 4);

    QVariant ageData = _exampleModel->data(ageIndex);
    QVariant networthData = _exampleModel->data(networthIndex);
    QVariant nameData = _exampleModel->data(nameIndex);

    ui->label_2->setText(nameData.toString());
    ui->label_7->setText('$' + networthData.toString() + " B");
